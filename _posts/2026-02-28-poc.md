---
comments: false
title: "PoC about CVE-2023-0461"
date: 2026-02-28 10:30
categories: [PoC, UAF]
tags: [x86-64, linux-kernel, cve-2023-0461, double-free, use-after-free]
---
## CVE Description
...
사용자가 연결 해제된 소켓의 fd를 `listen()` 함수의 인자로 전달하여 LISTEN 상태로 전환하고 다시 **connect** 할 때, 커널은 SYN 패킷을 처리하는 과정에서 이전에 사용되었던 소켓 데이터를 복제하여 새로운 소켓을 생성합니다. 

새로운 소켓에 상속된 `icsk`로 인해 `tls_context` 객체를 가리키는 포인터가 복제되고, 이로 인해 부모 소켓을 해제하면 자식 소켓은 해제된 `tls_context` 객체를 참조합니다.
...
반대로 `connect()` 호출 시 `addr.sin_family`에 AF_INET를 넘겨주면 됩니다.
...
```
커널의 `tcp_disconnect()` 에서 볼 수 있듯 `icsk` 객체를 초기화할 때 `icsk_ulp_data` 필드를 NULL로 변경하지 않습니다. 참고로, `disconnect()` 에서 복귀한 후에는 `sock→state`가 `SS_UNCONNECTED`로 설정됩니다.
...
LISTEN 상태로 소켓을 전환할 때 `inet_csk_listen_start()`가 `icsk->icsk_accept_queue`를 초기화하고 소켓을 `TCP_LISTEN`으로 전환합니다.
...
`sk_prot_alloc()`에서 복제본이 저장될 `sock` 객체를 할당하고 `sock_copy()`를 호출해 listener 소켓을 복제합니다. 복제될 때 기존 listener 소켓의 `icsk` 구조체까지 그대로 복제됩니다. 그 이후에 자식 소켓을 `icsk_accept_queue` 연결 리스트에 `req→sk` 필드에 저장합니다.
...
마지막으로 소켓을 close 할 때 TLS 컨텍스트가 어떻게 해제되는지 확인합니다.
```c
...
소켓을 `close()` 하는 과정에서 TLS 컨텍스트는 해제되지만 `icsk→icsk_ulp_data`의 포인터는 NULL로 초기화되지 않는 것을 확인할 수 있습니다. 이때 `tls_context` 객체를 해제하는 과정에서 `kfree_rcu()` 매크로를 사용하게 되는데, 해당 매크로는 `kvfree_call_rcu()`를 호출합니다. 
...
`kvfree_call_rcu()`에서 볼 수 있듯이 해제 요청을 받은 `tls_context` 객체를 즉시 해제하지 않고 RCU grace period  이후에 해제될 수 있도록 worker를 예약합니다. 커널은 다른 CPU에서 동시에 접근할 수 있는 데이터에 대한 reader가 완료됨을 보장할 수 있도록 유예 기간을 주는데, 이를 Read-Copy-Update(RCU) grace period라고 합니다. 
...
이 목차에서는 `CONFIG_KASAN`이 활성화된 커널에서 PoC 코드를 작성하는 과정을 다룹니다. 로컬에서 서버와 클라이언트 역할을 수행할 두 개의 소켓을 준비하고, 소켓에 TLS를 설치하는 코드를 먼저 구현합니다.
...
다음은 연결된 소켓을 연결 해제하고 listener로 전환한 뒤 다시 연결하는 과정입니다.
...
`connect()` 과정에서 SYN 패킷을 커널이 수신하고 이전의 소켓 데이터가 복제되어 자식 소켓에게 `tls_context`가 상속되었습니다. 다음은 부모 소켓을 `close()`를 사용해 해제하여 자식 소켓에서 UAF 객체를 획득하는 과정입니다.
...
RCU grace period로 설정된 5초 이상을 `sleep()`으로 대기하여 `kvfree_call_rcu()`으로 지연된 `tls_context` 객체의 해제 작업이 완료될 수 있도록 합니다. 그리고 자식 소켓을 `close()`하면 해제된 `tls_context`가 다시 해제되면서 KASAN이 double-free 버그를 출력하게 됩니다.

KASAN은 커널이 어떤 메모리를 참조하기 전에 먼저 해당 메모리의 상태를 검사하고 해제된 객체에 접근하려는 경우 use-after-free, 또는 double-free 로그를 출력합니다. 따라서 유저 영역에서 해제된 `tls_context` 객체에 접근할 수 있는 코드를 실행하면 UAF 로그가 출력됩니다.

[Kernel subsystem 문서](https://www.kernel.org/doc/html/v6.1/networking/tls.html)에서는 `tls12_crypto_info_aes_gcm_128` 구조체를 설정하고 `setsockopt()`의 옵션으로 `TLS_TX`, `TLS_RX`를 전달합니다. 소켓 옵션이 적용되면 해당 소켓을 통해 송수신되는 모든 데이터는 TLS와 소켓 옵션에 제공된 매개변수를 사용하여 암호화됩니다. 즉, 커널이 송수신을 처리하는 과정에서 `tls_context` 객체에 접근합니다.

문서의 예제 코드를 참고하여 작성한 전체 PoC 코드는 제 [깃허브](https://github.com/gimmia/1-day-study/blob/main/linux_kernel/CVE/2023/0461/poc/poc.c)에 있습니다.
...
실제로 `send()`를 처리하는 과정에서 UAF 로그가 출력되었습니다. 동적 분석으로 UAF가 발생한 지점을 찾기 전에 소스코드를 먼저 확인하여 흐름을 파악해두는 것이 좋습니다.
...
이때 TLS가 적용된 소켓은 kTLS 모듈에 정의된 전용 함수가 있기 때문에 `sendmsg()` 함수 포인터로 점프하여 `tls_sw_sendmsg()`를 호출하게 되고, `mutex_lock()`에서 UAF가 발생합니다.
...
실제로 `mutex_lock()`은 해제된 `tls_context`를 참조에 사용하고 있습니다.

## Debugging

KASAN 로그는 추적된 함수들의 옆에 offset 정보를 포함합니다. 이 정보를 바탕으로 함수의 진입점을 쉽게 찾을 수 있습니다. 

**sock_sendmsg+0x8e**

![image.png](/assets/images/sock_sendmsg.png)
...
![image.png](/assets/images/inet_sendmsg.png)
...
`inet_sendmsg+0x3e`에는 `sk->sk_prot->sendmsg()` 함수 포인터를 `$RAX`에 저장하는 코드가 있습니다. 그리고 이 시점에서 `$RDI`에는 `inet_send_prepare()`가 호출될 때 인자로 전달했던 자식 소켓 주소가 아직 남아있습니다. 
...
![image.png](/assets/images/tls_sw_sendmsg.png)
...
이후 실행에서 앞서 보았던 `KASAN: use-after-free` 로그가 출력되는 것을 확인할 수 있습니다. 

## 마치며

이 글에서는 리눅스 커널 6.1.0 버전에 존재하는 CVE-2023-0461(UAF) 취약점이 어떻게 동작하는지 간단하게 살펴보고, PoC 코드를 만들어 KASAN log를 출력하는 과정까지 설명했습니다. 

소개드린 취약점은 공격자가 객체를 간단히 두 번 해제할 수 있어서 UAF 객체를 만들기 쉽습니다. `CONFIG_SLAB_VIRTUAL`이나 `CONFIG_KMALLOC_SPLIT_VARSIZE` 같은 실험적 완화 조치가 적용되지 않은 환경(예: kCTF)이라면 cross-cache attack이나 dirty pagetable 같은 여러 공격 기법들을 공부하기 좋은 조건이라고 생각합니다.

다음 블로그 게시물에서는 익스플로잇 과정을 다루도록 하겠습니다.
